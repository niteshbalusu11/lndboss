...
APPID="lndboss" # one-word lower-case no-specials

# the git repo to get the source code from for install
GITHUB_REPO="https://github.com/niteshbalusu11/lndboss"
...
if [ "$1" = "1" ] || [ "$1" = "on" ]; then
  ...
  if ! [ -d /mnt/hdd/app-data/${APPID} ]; then
    ...
    sudo rm -rf /home/bos/.bosgui # delete any old folder because symlink needs to be created
    sudo ln -s /mnt/hdd/app-data/${APPID}/ /home/${APPID}/.bosgui
  ...
  # set up npm-global
  sudo -u ${APPID} mkdir /home/${APPID}/.npm-global
  sudo -u ${APPID} npm config set prefix '/home/lndboss/.npm-global'
  sudo bash -c "echo 'PATH=$PATH:/home/lndboss/.npm-global/bin' >> /home/lndboss/.bashrc"


  # Install yarn
  echo "# Installing yarn package manager"
  sudo -u ${APPID} npm i -g yarn

  # make sure needed debian packages are installed
  # 'fbi' is here just an example - change to what you need or delete
  echo "# install lndboss from npm"
  /home/${APPID}/.npm-global/bin/yarn global add lndboss
...
[Service]
ExecStart=/home/${APPID}/.npm-global/bin/yarn --cwd /home/lndboss/.config/yarn/global/node_modules/lndboss start:prod
User=${APPID}
Restart=always
TimeoutSec=120
RestartSec=30
...
if [ "$1" = "0" ] || [ "$1" = "off" ]; then
  ...
  yarn global remove lndboss

  echo "Deleting user lndboss"
  sudo deluser --remove-home ${APPID}
